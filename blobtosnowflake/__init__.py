...
try:
            ...
            # Download the latest file from Blob Storage
                ...
                if(len(updated_column_list) != 0):
                    if(len(updated_column_list) != len(column_dict)):
                        blob_table_column_list = list(sorted(set(list(column_dict.keys())) - set(updated_column_list)))
                        for column in enumerate(blob_table_column_list):
    ...
    if len(failed) == 0:
            response = {
                'statusCode': 200,
                'body': json.dumps(f"The function succeeded and updated {len(tables_list)} tables")
            }
    else:
        response = {
            'statusCode': 500,
